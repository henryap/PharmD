...
function optionsframework_validate($input) {
	...
	return of_get_default_values($input);
    }
 ...
 * @access    private
 */
function of_get_default_values($input=null) {
    $output = array();
    $config = optionsframework_options();
    foreach ((array) $config as $option) {
	// checks to see if the option is superadmin only, if so, ignore reset
	if ($option['super-admin-only'] == "1" && !is_super_admin() && isset($input)) {
	    $id = preg_replace('/\W/', '', strtolower($option['id']));
	    $output[$option['id']] = apply_filters('of_sanitize_' . $option['type'], $input[$id], $option);
	} else {
	    if (!isset($option['id'])) {
		continue;
	    }
