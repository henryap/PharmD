...
function optionsframework_fields() {
	...
	if (($value['type'] != "heading") && ($value['type'] != "info")) {
	    ...
	    $output .= '<div id="' . esc_attr($id) . '" class="' . esc_attr($class) . ' ' . ($value['super-admin-only'] == '1' && !is_super_admin() ? 'hideField' : '') . '">' . "\n";
	...
	switch ($value['type']) {
		...
		// url field
        case 'url':
		$output .= '<input id="' . esc_attr($value['id']) . '" class="of-input" name="' . esc_attr($option_name . '[' . $value['id'] . ']') . '" type="text" value="' . htmlspecialchars($val, ENT_QUOTES) . '" />';
		break;
		...
		$menu .= '<li ' . ($value['super-admin-only'] == '1' && !is_super_admin() ? 'class="hideField" ' : '') . '><a id="' . esc_attr($jquery_click_hook) . '-tab" title="' . esc_attr($value['name']) . '" href="' . esc_attr('#' . $jquery_click_hook) . '">' . esc_html($value['name']) . '</a></li>';
		$output .= '<div class="' . ($value['super-admin-only'] == '1' && !is_super_admin() ? 'hideField' : 'group') . '" id="' . esc_attr($jquery_click_hook) . '"><h2>' . esc_html($value['name']) . '</h2>' . "\n";
		break;
	}
